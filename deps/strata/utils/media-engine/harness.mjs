// media-engine/harness.mjs
// The tiny zero-dependency test harness shared by every unit-test file —
// the same shape as virtualization-engine's inline harness, factored out so a suite can
// span many files and still report once.
//
...
// module (so a single file run directly prints and exits). media-engine/run-all.mjs
// imports every test file (their tests execute on import) and then calls
// report() once for the aggregate.
