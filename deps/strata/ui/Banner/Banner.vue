...
<template>
  <div :role="role === 'none' ? undefined : role" :class="['flex gap-3 rounded-large border p-4', cls]">
    ...
    <button
      ...
      class="-m-1.5 grid size-7 shrink-0 place-items-center self-start rounded-small opacity-70 transition-opacity duration-100 hover:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground/30"
      @click="emit('dismiss')"
    >
