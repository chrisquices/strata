...
const value = computed<CalendarDate | undefined>({
  get: function () { return dateFromIso(model.value); },
  set: function (date) {
...
const hasValue = computed(function () { return !!dateFromIso(model.value); });
...
const cellTrigger = 'flex h-10 flex-1 cursor-pointer items-center justify-center rounded-medium text-sm text-foreground transition-colors duration-100 select-none hover:bg-border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-foreground/40 data-[today]:font-semibold data-[today]:ring-1 data-[today]:ring-border data-[selected]:bg-foreground data-[selected]:text-background data-[selected]:ring-0 data-[selected]:hover:bg-foreground data-[disabled]:cursor-not-allowed data-[disabled]:text-faint data-[disabled]:opacity-40 data-[disabled]:hover:bg-transparent';
</script>
      ...
      >
        ...
        >
          <MonthPickerHeader class="flex items-center justify-between pb-2">
            <MonthPickerPrev :class="navigationButton"><ChevronLeft class="size-icon-small" aria-hidden="true" /></MonthPickerPrev>
            <MonthPickerHeading class="text-sm font-medium text-foreground" />
            <MonthPickerNext :class="navigationButton"><ChevronRight class="size-icon-small" aria-hidden="true" /></MonthPickerNext>
          </MonthPickerHeader>
          <MonthPickerGrid class="w-full border-collapse">
            <MonthPickerGridBody>
              <MonthPickerGridRow v-for="(row, index) in grid.rows" :key="index" class="flex gap-1">
                <MonthPickerCell v-for="month in row" :key="month.toString()" :date="month" class="flex-1">
                  <MonthPickerCellTrigger v-slot="{ monthValue }" :month="month" :class="cellTrigger">{{ monthValue }}</MonthPickerCellTrigger>
                </MonthPickerCell>
              </MonthPickerGridRow>
            </MonthPickerGridBody>
          </MonthPickerGrid>
