...
const sizeClass = computed(function () {
  return { sm: 'px-2.5 text-xs', md: 'px-3 text-sm', lg: 'px-3.5 text-base' }[style.size] ?? 'px-3 text-sm';
});
const stateClass = computed(function () {
  return style.variant === 'secondary'
...
<template>
  <ToggleGroupItem
    ...
    :disabled="disabled"
    :class="[
      'inline-flex h-full items-center justify-center gap-1.5 rounded-small transition-colors duration-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground/30 select-none',
      sizeClass,
      stateClass,
