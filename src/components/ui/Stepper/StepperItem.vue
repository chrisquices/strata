  ...
  <StepperItem v-slot="{ state }" :step="step" class="group">
    ...
    >
      <div class="flex flex-col items-center">
        <StepperIndicator
          :class="['flex size-6 shrink-0 items-center justify-center rounded-full text-xs font-medium transition-colors duration-base', indicatorClass(state)]"
        >
      ...
      <div class="min-w-0 pb-6 group-last:pb-0">
        <StepperTitle :class="['block text-sm leading-tight transition-colors duration-base', titleClass(state)]">{{ title }}</StepperTitle>
        <StepperDescription
          v-if="description"
          :class="['mt-0.5 block text-xs leading-tight transition-colors duration-base', descriptionClass(state)]"
