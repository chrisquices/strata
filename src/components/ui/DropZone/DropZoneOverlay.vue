...
const dragging = computed(function () {
  return depth.value > 0 && !props.disabled;
});
...
<template>
  ...
  <div
      :class="['pointer-events-none fixed inset-0 z-modal flex items-center justify-center bg-overlay/40 p-overlay backdrop-blur-sm transition-opacity duration-200', dragging ? 'opacity-100' : 'opacity-0']"
      :aria-hidden="!dragging"
  >
    <slot v-if="$slots.overlay" name="overlay"/>
    <div
        v-else
        :class="['flex flex-col items-center gap-cluster rounded-large border-medium border-dashed border-foreground/40 bg-surface px-overlay-large py-overlay-large text-center shadow-panel transition-transform duration-200 ease-out', dragging ? 'scale-100' : 'scale-95']"
    >
