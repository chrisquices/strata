...
<template>
  ...
  <div
    :class="['pointer-events-none fixed inset-0 z-modal flex items-center justify-center bg-overlay/40 p-6 backdrop-blur-sm transition-opacity duration-base', dragging ? 'opacity-100' : 'opacity-0']"
    :aria-hidden="!dragging"
  >
    <slot v-if="$slots.overlay" name="overlay" />
    <div
      v-else
      :class="['flex flex-col items-center gap-3 rounded-large border-medium border-dashed border-foreground/40 bg-surface px-12 py-10 text-center shadow-panel transition-transform duration-base ease-out', dragging ? 'scale-100' : 'scale-95']"
    >
