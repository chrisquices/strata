...
provide('inputGroup', reactive({
  size: computed(function () { return props.size; }),
  invalid: computed(function () { return props.invalid || !!groupError.value; }),
  errorId: groupErrorId,
  registerError: function (msg: string) { groupError.value = msg; },
  clearError: function () { groupError.value = ''; },
}));

const sizeClass = {sm: 'h-control-small text-xs', md: 'h-control text-sm', lg: 'h-control-large text-base'};
const isInvalid = computed(function () { return props.invalid || !!groupError.value; });
...
<template>
  <FieldErrorTooltip :id="groupErrorId" :message="groupError">
    <div
        :class="['flex overflow-hidden rounded-medium border transition-colors duration-100', borderClass, sizeClass[size] ?? sizeClass.md]">
      <slot/>
    </div>
  </FieldErrorTooltip>
</template>
