...
function onFiles(files) { accepted.value = files.map(f => f.name); rejected.value = false; }
function onRejected() { accepted.value = []; rejected.value = true; }
</script>
  ...
  <div class="flex flex-col gap-14">
    <ComponentItemSection>
      ...
      <ComponentItemSectionDescription>
        accept="image/*". {{ rejected ? 'Last drop rejected.' : accepted.length ? 'Accepted: ' + accepted.join(', ') : 'Drop or click to browse.' }}
      </ComponentItemSectionDescription>
    ...
    <ComponentItemSection>
      ...
      <ComponentItemSectionDescription>
        DropZoneOverlay shows a full-window prompt whenever files are dragged anywhere over the page. Files dropped: {{ overlayDropped }}
      </ComponentItemSectionDescription>
      <ComponentItemSectionExample>
